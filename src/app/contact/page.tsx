import type { Metadata } from "next";
import Link from "next/link";

export const metadata: Metadata = {
    title: "Contact Jayesh Tambe - Senior Software Engineer",
    description: "Connect with Jayesh Tambe for collaboration, project opportunities, technical discussions, or consulting. Download resume and find social links.",
    keywords: [
        "Contact",
        "Jayesh Tambe",
        "Senior Software Engineer",
        "Hiring",
        "Collaboration",
        "LinkedIn",
        "GitHub",
    ],
    openGraph: {
        title: "Contact Jayesh Tambe - Senior Software Engineer",
        description: "Connect with Jayesh Tambe for collaboration and opportunities.",
        type: "website",
        url: "https://jtambe.github.io/contact",
    },
};

export default function ContactPage() {
    return (
        <main className="mt-24 px-6 py-12 max-w-2xl mx-auto">
            <h1 className="text-4xl font-bold mb-2 text-gray-800 dark:text-white">Contact Me</h1>
            <p className="text-gray-600 dark:text-gray-300 mb-12">
                I'm always open to new opportunities, interesting projects, and conversations about technology. 
                Feel free to reach out or download my resume below.
            </p>

            {/* Resume Download Section */}
            <section className="mb-12 p-6 bg-blue-50 dark:bg-blue-900 rounded-lg border border-blue-200 dark:border-blue-700">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-4">Resume</h2>
                <p className="text-gray-700 dark:text-gray-300 mb-6">
                    Download my resume to learn more about my professional experience and qualifications.
                </p>
                <a
                    href="/Jayesh_Tambe_Resume.pdf"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-block px-6 py-3 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 
                    text-white font-semibold rounded-lg transition-colors"
                >
                    Resume
                </a>
            </section>

            {/* Contact Information */}
            <section className="space-y-8">
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Get in Touch</h2>

                <div className="space-y-6">
                    {/* Email */}
                    {/* <div className="flex items-start gap-4">
                        <div className="flex-shrink-0">
                            <svg className="w-6 h-6 text-blue-600 dark:text-blue-400 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                            </svg>
                        </div>
                        <div>
                            <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-1">Email</h3>
                            <a
                                href="mailto:jayesh@example.com"
                                className="text-blue-600 dark:text-blue-400 hover:underline break-all"
                            >
                                jayesh@example.com
                            </a>
                        </div>
                    </div> */}

                    {/* LinkedIn */}
                    <div className="flex items-start gap-4">
                        <div className="flex-shrink-0">
                            <svg className="w-6 h-6 text-blue-600 dark:text-blue-400 mt-1" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.469v6.766z" />
                            </svg>
                        </div>
                        <div>
                            <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-1">LinkedIn</h3>
                            <a
                                href="https://linkedin.com/in/jayeshtambe"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-blue-600 dark:text-blue-400 hover:underline"
                            >
                                linkedin.com/in/jayeshtambe
                            </a>
                        </div>
                    </div>

                    {/* GitHub */}
                    <div className="flex items-start gap-4">
                        <div className="flex-shrink-0">
                            <svg className="w-6 h-6 text-gray-800 dark:text-white mt-1" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v 3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                            </svg>
                        </div>
                        <div>
                            <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-1">GitHub</h3>
                            <a
                                href="https://github.com/jtambe"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-blue-600 dark:text-blue-400 hover:underline"
                            >
                                github.com/jtambe
                            </a>
                        </div>
                    </div>

                    {/* Medium */}
                    <div className="flex items-start gap-4">
                        <div className="flex-shrink-0">
                            <svg className="w-6 h-6 text-gray-800 dark:text-white mt-1" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M13.54 12a6.8 6.8 0 01-6.77 6.82A6.8 6.8 0 010 12a6.8 6.8 0 016.77-6.82A6.8 6.8 0 0113.54 12zM20.96 12c0 3.54-1.51 6.42-3.38 6.42-1.87 0-3.39-2.88-3.39-6.42s1.52-6.42 3.39-6.42 3.38 2.88 3.38 6.42M24 12c0 3.17-.53 5.75-1.19 5.75-.59 0-1.06-2.44-1.06-5.75s.47-5.75 1.06-5.75C23.47 6.25 24 8.83 24 12z" />
                            </svg>
                        </div>
                        <div>
                            <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-1">Medium</h3>
                            <a
                                href="https://medium.com/@jayeshtambe"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-blue-600 dark:text-blue-400 hover:underline"
                            >
                                medium.com/@jayeshtambe
                            </a>
                        </div>
                    </div>
                </div>
            </section>

            {/* Back to Home */}
            <div className="mt-12 pt-6 border-t border-gray-200 dark:border-gray-700">
                <Link href="/" className="text-blue-600 dark:text-blue-400 hover:underline">
                    ← Back to Home
                </Link>
            </div>
        </main>
    );
}
